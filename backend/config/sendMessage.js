...
function getHeaderFooter() {
    try {
        ...
        return {
            header: settings.company_header || 'AXELINK',
            footer: settings.footer_info || 'Powered by Axelink'
        };
    } catch (error) {
        return {
            header: 'AXELINK',
            footer: 'Powered by Axelink'
        };
    }
}
...
async function sendMessage(number, message) {
    // Fallback to global sock if local not set
    if (!sock && global.whatsappSock) {
        sock = global.whatsappSock;
    }
    if (!sock) {
    ...
    try {
        ...
        for (const number of numberArray) {
            try {
                ...
                // Cek apakah nomor terdaftar di WhatsApp (hanya jika API tersedia)
                if (typeof sock.onWhatsApp === 'function') {
                    try {
                        ...
                        console.warn(`onWhatsApp check failed for ${cleanNumber}, proceeding to send anyway (fallback mode): ${e.message}`);
                    }
                } else {
                    // Fallback client (whatsapp-web.js) does not support onWhatsApp; proceed to send
                    console.log(`onWhatsApp not available, attempting send to ${cleanNumber}`);
                }
...
async function sendTechnicianMessage(message, priority = 'normal') {
    try {
        // Ambil daftar nomor teknisi dari env atau settings
        const { getSetting } = require('./settingsManager');
        let technicianNumbers = [];
        
        // Prioritas: process.env.TECHNICIAN_NUMBERS (comma separated)
        if (process.env.TECHNICIAN_NUMBERS && process.env.TECHNICIAN_NUMBERS.trim()) {
            technicianNumbers = process.env.TECHNICIAN_NUMBERS.split(',').map(n => n.trim()).filter(Boolean);
        } else {
            // Fallback ke settings.json berindeks
            let i = 0;
            while (true) {
        ...
        const technicianGroupId = process.env.TECHNICIAN_GROUP_ID || getSetting('technician_group_id', '');
        ...
        if (technicianNumbers && technicianNumbers.length > 0) {
            ...
            const adminNumber = process.env.WHATSAPP_ADMINS || getSetting('admins.0', '');
            if (adminNumber) {
