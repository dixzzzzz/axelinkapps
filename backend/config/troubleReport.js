...
async function sendNotificationToTechnicians(report) {
  try {
    ...
    const companyHeader = process.env.COMPANY_HEADER || getSetting('company_header', 'AxeLink');
...
async function sendStatusUpdateToCustomer(report) {
  try {
    ...
    // Gunakan nomor telepon mentah; biarkan pengirim yang memformat
    const customerPhone = String(report.phone);
    logger.info(`Nomor pelanggan untuk update status: ${customerPhone}`);
...
Jika ada pertanyaan, silakan hubungi kami.`;
    ...
    if (result) {
      ...
      // Fallback: kirim ke admin untuk notifikasi manual
      try {
        const adminNumber = process.env.WHATSAPP_ADMINS || getSetting('admins.0', '');
        if (adminNumber) {
          await sendMessage(adminNumber, `⚠️ Gagal mengirim update status ke pelanggan ${report.phone} untuk tiket ${report.id}. Mohon tindak lanjut manual.\n\n${message}`);
        }
      } catch (fallbackErr) {
        logger.error(`❌ Fallback admin notification failed: ${fallbackErr.message}`);
      }
      return false;
    }
  } catch (error) {
