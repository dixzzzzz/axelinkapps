    ...
    try {
        ...
        if (mikrotikConnection) {
            try {
                ...
                await Promise.race([
                    mikrotikConnection.write('/system/resource/print'),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Connection test timeout')), 3000))
                ]);
                return mikrotikConnection;
            } catch (err) {
                logger.warn(`Existing Mikrotik connection failed, reconnecting: ${err.message}`);
                mikrotikConnection = null;
            }
        }
        
        // Buat koneksi baru jika tidak ada atau koneksi lama tidak valid
        return await connectToMikrotik();
    } catch (error) {
        logger.error(`Error in getMikrotikConnection: ${error.message}`);
        throw new Error(`Gagal mendapatkan koneksi Mikrotik: ${error.message}`);
    }
}
...
async function getPPPoEUsers() {
    ...
    if (mode === 'radius') {
        ...
        const activeArray = (activeResult && activeResult.success && Array.isArray(activeResult.data)) ? activeResult.data : [];

        // Build a map of active connections keyed by username/name for quick lookup
        const activeMap = {};
        for (const a of activeArray) {
            if (a && (a.name || a.user)) {
        ...
        // Try to read interface traffic stats and map to PPPoE users.
        // Some RouterOS setups expose PPPoE per-user interfaces named like '<pppoe-USERNAME>' or 'pppoe-USERNAME'.
        try {
            const ifaceStats = await conn.write('/interface/print', ['=stats=detail']);
            if (Array.isArray(ifaceStats) && ifaceStats.length > 0) {
                for (const iface of ifaceStats) {
                    try {
                        const inNames = Object.keys(iface).map(k => k.toLowerCase());
                        // Look for fields that indicate RX/TX bytes
                        const rxField = Object.keys(iface).find(k => /rx-?byte|rx-?bytes|rx$/i.test(k));
                        const txField = Object.keys(iface).find(k => /tx-?byte|tx-?bytes|tx$/i.test(k));
                        if (!rxField || !txField) continue;

                        const rawName = String(iface.name || iface['name'] || '');
                        // Normalize interface name: examples: '<pppoe-TEH VANI>' or 'pppoe-TEH VANI'
                        const m = rawName.match(/pppoe[-<]?\s*([^>]+)?>?/i) || rawName.match(/^pppoe-(.+)$/i);
                        let username = null;
                        if (m && m[1]) {
                            username = m[1].trim();
                        } else if (/pppoe/i.test(rawName)) {
                            // fallback: remove non-alphanum
                            username = rawName.replace(/[^a-z0-9\-\s_]/ig, '').replace(/^pppoe[-\s_]?/i, '').trim();
                        }
                        if (!username) continue;

                        // Parse numbers (RouterOS prints numbers with spaces as thousand separators)
                        const parseNum = v => {
                        ...
                        const rx = parseNum(iface[rxField]);
                        const tx = parseNum(iface[txField]);

                        // Merge into activeMap if username key exists
                        if (username && activeMap[username]) {
                            activeMap[username] = Object.assign({}, activeMap[username], { 'bytes-in': rx, 'bytes-out': tx });
                        }
                    } catch (mapErr) {
                        // ignore mapping errors per-interface
                    }
                }
            }
        } catch (ifaceErr) {
            // ignore interface stats failures
        }

    // If active entries are missing byte counters, try fetching details per active user
    // (do this even if address is present) but cap the number of per-entry queries to avoid overloading the router
    const needsDetail = activeArray.some(a => !(a['bytes-in'] || a['bytes-out'] || a['rx-byte'] || a['tx-byte'] || a['rx-bytes'] || a['tx-bytes']));
        if (needsDetail && activeArray.length > 0) {
            const cap = Math.min(activeArray.length, 100); // cap to 100 entries
            try {
                const limited = activeArray.slice(0, cap);
                for (const item of limited) {
                    const name = item.name || item.user;
                    if (!name) continue;
                    try {
                        // Query active entry filtered by name requesting common props
                        const detail = await conn.write('/ppp/active/print', [
                            `?name=${name}`,
                            '=.proplist=name,uptime,address,remote-address,caller-id,bytes-in,bytes-out,rx-byte,tx-byte,rx-bytes,tx-bytes'
                        ]);
                        if (Array.isArray(detail) && detail.length > 0) {
                            // Merge returned detail into our activeMap entry
                            const d = detail[0];
                            const key = name;
                            activeMap[key] = Object.assign({}, activeMap[key] || {}, d);
                        }
                    } catch (perErr) {
                        // ignore per-entry failures but continue
                        logger.debug(`[mikrotik] per-entry active details failed for ${name}: ${perErr.message}`);
                    }
                }
            } catch (qErr) {
                logger.warn('[mikrotik] detailed active lookup failed:', qErr.message);
            }
        }

            // Additional debug: log full activeMap objects for first few active users where bytes are missing
            try {
                if (process.env.NODE_ENV !== 'production') {
                    const missingBytesSamples = [];
                    for (const [key, val] of Object.entries(activeMap)) {
                        const bytesInVal = val && (val['bytes-in'] || val['rx-byte'] || val['rx-bytes'] || val['bytes_in'] || val['rate-in']);
                        const bytesOutVal = val && (val['bytes-out'] || val['tx-byte'] || val['tx-bytes'] || val['bytes_out'] || val['rate-out']);
                        if (!bytesInVal && !bytesOutVal) {
                            missingBytesSamples.push({ name: key, full: val });
                        }
                        if (missingBytesSamples.length >= 5) break;
                    }
                    if (missingBytesSamples.length > 0) {
                    }
                }
            } catch (dbgErr3) {}

        // Development debug: log a small sample of active connections and their keys
        try {
            if (process.env.NODE_ENV !== 'production') {
                logger.info(`[mikrotik] active connections count: ${activeArray.length}`);
                const sample = activeArray.length > 0 ? activeArray.slice(0, 5).map(a => ({
                    name: a.name || a.user || a['.id'] || null,
                    keys: Object.keys(a || {}).slice(0, 20),
                    address: a.address || a['remote-address'] || a['caller-id'] || null,
                    bytesIn: a['bytes-in'] || a['rx-byte'] || a['rx-bytes'] || null,
                    bytesOut: a['bytes-out'] || a['tx-byte'] || a['tx-bytes'] || null
                })) : [];
            }
        } catch (dbgErr) {
            logger.warn(`[mikrotik] debug sample failed: ${dbgErr.message}`);
        }

        // Development debug: log PPP secrets count and a small sample so we can compare
        try {
            if (process.env.NODE_ENV !== 'production') {
                const secretSample = pppSecrets.length > 0 ? pppSecrets.slice(0, 5).map(s => ({
                    id: s['.id'],
                    name: s.name,
            ...
            logger.warn(`[mikrotik] pppSecrets debug failed: ${dbgErr2.message}`);
        }

        // Gabungkan data: attach active connection details when available
        return pppSecrets.map(secret => {
            const active = activeMap[secret.name] || null;

            // Possible fields for IP and byte counters vary by RouterOS/version; check multiple keys
            // Prefer active connection IPs; otherwise use secret's local/remote address
            const ipAddress = active?.address || active?.['remote-address'] || active?.['caller-id'] || active?.['caller-id-address'] || secret['local-address'] || secret['remote-address'] || null;
            const uptime = active?.uptime || active?.['uptime'] || null;
            const bytesIn = safeNumber(active?.['bytes-in'] || active?.['rx-byte'] || active?.['rx-bytes'] || active?.['rx-bytes-total']);
            const bytesOut = safeNumber(active?.['bytes-out'] || active?.['tx-byte'] || active?.['tx-bytes'] || active?.['tx-bytes-total']);
            ...
            const obj = {
                ...
                active: !!active,
                lastLogin: secret['last-logged-out'] || null,
            ...
            // Debug when active but no ip/counters found for this user
            if (process.env.NODE_ENV !== 'production' && obj.active && (!obj.ipAddress || (!obj.bytesIn && !obj.bytesOut))) 

            // If bytes are missing, attempt to use usageCache by ip address (best-effort)
            try {
                if ((!obj.bytesIn || !obj.bytesOut) && obj.ipAddress) {
                    const cached = usageCache[obj.ipAddress] || usageCache[obj.ipAddress + ':0'];
                    if (cached) {
                        obj.bytesIn = obj.bytesIn || cached.rx || 0;
                        obj.bytesOut = obj.bytesOut || cached.tx || 0;
                    }
                }
            } catch (useErr) {
                // ignore
...
async function editPPPoEUser({ id, username, password, profile, remoteAddress, localAddress }) {
    try {
        const conn = await getMikrotikConnection();
        if (!conn) throw new Error('Koneksi ke Mikrotik gagal');
        
        // Validasi parameter yang diperlukan
        if (!id) throw new Error('ID PPPoE user tidak boleh kosong');
        if (!username) throw new Error('Username PPPoE tidak boleh kosong');
        if (!profile) throw new Error('Profile PPPoE tidak boleh kosong');
        
        // Buat argumen untuk perintah Mikrotik
        const args = [
            '=.id=' + id,
            '=name=' + username,
            '=profile=' + profile
        ...
        // Include remote-address if provided (non-empty)
        if (remoteAddress !== undefined && remoteAddress !== null && String(remoteAddress).length > 0) {
            args.push('=remote-address=' + remoteAddress);
        }
        
        // Include local-address if provided (non-empty)
        if (localAddress !== undefined && localAddress !== null && String(localAddress).length > 0) {
            args.push('=local-address=' + localAddress);
        }
        
        // Eksekusi perintah ke Mikrotik dengan timeout
        const result = await Promise.race([
            conn.write('/ppp/secret/set', args),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout: Koneksi ke Mikrotik terlalu lama')), 10000))
        ]);
        
        logger.info(`PPPoE user ${username} berhasil diupdate`);
        return { success: true };
    } catch (error) {
        logger.error(`Error editing PPPoE user: ${error.message}`);
        // Tambahkan informasi detail untuk debugging
        logger.error(`Detail error: ${JSON.stringify({
            id,
            username,
            profileProvided: !!profile,
            passwordProvided: !!(password && password.length > 0),
            remoteAddressProvided: !!(remoteAddress && remoteAddress.length > 0),
            localAddressProvided: !!(localAddress && localAddress.length > 0),
...
async function getActivePPPoEConnections() {
    try {
        ...
        // Request specific properties to ensure IP and byte counters are included when available
        let pppConnections;
        try {
            pppConnections = await conn.write('/ppp/active/print', [
                '=.proplist=name,profile,uptime,address,remote-address,caller-id,bytes-in,bytes-out,rx-byte,tx-byte,rx-bytes,tx-bytes'
            ]);
        } catch (e) {
            // Fallback to generic print if the router doesn't support proplist parameter
            pppConnections = await conn.write('/ppp/active/print');
        }
        return {
            success: true,
...
async function getInterfaceTraffic(interfaceName = 'ether1') {
    try {
        ...
        let res;
        try {
            // If interface name contains spaces or special chars, quote it for RouterOS API
            const needsQuotes = /[\s\"']/g.test(interfaceName);
            const ifaceArg = needsQuotes ? `=interface="${String(interfaceName).replace(/"/g, '\\"')}"` : `=interface=${interfaceName}`;
            res = await conn.write('/interface/monitor-traffic', [
                ifaceArg,
                '=once='
            ]);
        } catch (innerErr) {
            logger.error(`[mikrotik] monitor-traffic command failed for interface=${interfaceName}`, { error: innerErr && (innerErr.stack || innerErr.message || String(innerErr)) });

            // Fallback: try to resolve interface by exact name via /interface/print and attempt again using the returned name/id
            try {
                const allIfaces = await conn.write('/interface/print');

                // Normalize function: remove non-alphanumeric and lowercase
                const normalize = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
                const targetNorm = normalize(interfaceName);

                // Try exact name match first, then normalized name match
                let found = allIfaces.find(i => i.name === interfaceName || i['name'] === interfaceName);
                if (!found) {
                    found = allIfaces.find(i => normalize(i.name) === targetNorm);
                }

                if (found) {
                    const fallbackName = found.name || found['.id'];
                    const needsQ = /[\s\"']/g.test(fallbackName);
                    const fallbackArg = needsQ ? `=interface="${String(fallbackName).replace(/"/g, '\\"')}"` : `=interface=${fallbackName}`;
                    try {
                        res = await conn.write('/interface/monitor-traffic', [fallbackArg, '=once=']);
                    } catch (innerErr2) {
                        logger.error(`[mikrotik] monitor-traffic fallback also failed for interface=${interfaceName} using ${fallbackName}`, { error: innerErr2 && (innerErr2.stack || innerErr2.message || String(innerErr2)) });
                        return { rx: 0, tx: 0 };
                    }
                } else {
                    return { rx: 0, tx: 0 };
                ...
                logger.error(`[mikrotik] monitor-traffic fallback lookup failed for interface=${interfaceName}`, { error: lookupErr && (lookupErr.stack || lookupErr.message || String(lookupErr)) });
                return { rx: 0, tx: 0 };
            }
        }

        if (!res || !res[0]) {
            logger.warn(`[mikrotik] monitor-traffic returned empty for interface=${interfaceName}`, { res });
            return { rx: 0, tx: 0 };
        }

        const sample = res[0];

        // Try many possible keys for counters / rates (RouterOS variations)
        const possibleRxKeys = [
            'rx-bits-per-second', 'rx-bits', 'rx-rate', 'rate-in', 'rate', 'rx-byte', 'rx-bytes', 'bytes-in', 'rx', 'rx_bytes'
        ];
        const possibleTxKeys = [
            'tx-bits-per-second', 'tx-bits', 'tx-rate', 'rate-out', 'tx-byte', 'tx-bytes', 'bytes-out', 'tx', 'tx_bytes'
        ];
        ...
        const pickNumeric = (obj, keys) => {
            for (const k of keys) {
                if (obj[k] !== undefined && obj[k] !== null) {
                    ...
                    // Try to parse strings like "12345bps" or "12M"
                    const digits = String(obj[k]).match(/[-+]?\d*\.?\d+/);
        ...
        const rx = pickNumeric(sample, possibleRxKeys);
        const tx = pickNumeric(sample, possibleTxKeys);

        if (rx === 0 && tx === 0) {
            // Log sample for debugging when both counters are zero/missing
            logger.warn(`[mikrotik] monitor-traffic response missing expected numeric fields for interface=${interfaceName}`, { sample });
        }

        return { rx, tx };
    } catch (error) {
        // Serialize error including stack for better debugging
        try {
            const details = { message: error && error.message, stack: error && error.stack };
            logger.error('Error getting interface traffic:', details);
        } catch (e) {
            logger.error('Error getting interface traffic (and failed to serialize error):', String(error));
        }
        return { rx: 0, tx: 0 };
    }
}
