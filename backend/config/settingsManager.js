...
function getSetting(key, defaultValue) {
  ...
  // For admin credentials we intentionally do NOT fallback to settings.json.
  // The application uses .env for admin credentials only when present.
  if (key === 'admin_username' || key === 'admin_password') {
    return defaultValue;
  }

  // 2. Fallback ke settings.json for other keys
...
async function setHashedPassword(password) {
  try {
    ...
    // If ADMIN_PASSWORD is set in environment, we will not persist to settings.json
    // because credentials are managed via .env per deployment preference.
    if (process.env.ADMIN_PASSWORD) {
      console.warn('ADMIN_PASSWORD is set in environment. Not persisting new password to settings.json. Please update .env manually to change the admin password.');
      return false;
    }

    // Persist hashed password to settings.json only when env does not manage credentials
    return setSetting('admin_password', hashedPassword);
  } catch (error) {
...
async function verifyPassword(plainPassword) {
  try {
    ...
    // Fallback: stored value is not a bcrypt hash. Treat it as plain-text password.
    // WARNING: Storing plain-text passwords is insecure. We allow this for
    // backward compatibility or convenience, but log a warning.
    console.warn('Warning: admin_password is not a bcrypt hash. Falling back to plain-text comparison. Consider hashing the password for better security.');
    return plainPassword === String(hashedPassword);
  } catch (error) {
