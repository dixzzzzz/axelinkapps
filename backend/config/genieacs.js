        ...
        try {
            ...
            for (const [path, value] of Object.entries(parameters)) {
                ...
                if (path.includes('SSID')) {
                    // Cek apakah ini update untuk SSID 2.4GHz atau 5GHz
                    const isSSID2G = path.includes('WLANConfiguration.1');
                    const isSSID5G = path.includes('WLANConfiguration.5');
                    
                    if (isSSID2G) {
                        // Update SSID 2.4GHz saja
                        parameterPaths.ssid_2_4g.forEach(ssidPath => {
                            parameterValues.push([ssidPath, value, "xsd:string"]);
                        });
                        console.log(`Added SSID 2.4GHz parameters for ${onuType} ONU: "${value}"`);
                        console.log(`  - 2.4GHz paths: ${parameterPaths.ssid_2_4g.length} parameters`);
                    } else if (isSSID5G) {
                        // Update SSID 5GHz saja
                        parameterPaths.ssid_5g.forEach(ssidPath => {
                            parameterValues.push([ssidPath, value, "xsd:string"]);
                        });
                        console.log(`Added SSID 5GHz parameters for ${onuType} ONU: "${value}"`);
                        console.log(`  - 5GHz paths: ${parameterPaths.ssid_5g.length} parameters`);
                    } else {
                        // Fallback: jika path tidak jelas, gunakan logika lama (tapi ini seharusnya tidak terjadi)
                        console.warn(`⚠️  Ambiguous SSID path detected: ${path}, using fallback logic`);
                ...
                // Handle WiFi password update - gunakan paths yang spesifik untuk tipe ONU yang terdeteksi
                    ...
                    // Cek apakah ini update untuk password 2.4GHz atau 5GHz
                    const isPassword2G = path.includes('WLANConfiguration.1');
                    const isPassword5G = path.includes('WLANConfiguration.5');
                    
                    if (isPassword2G) {
                        // Update password 2.4GHz saja
                        parameterPaths.password_2_4g.forEach(passPath => {
                            parameterValues.push([passPath, value, "xsd:string"]);
                        });
                        console.log(`Added password 2.4GHz parameters for ${onuType} ONU`);
                        console.log(`  - 2.4GHz password paths: ${parameterPaths.password_2_4g.length} parameters`);
                    } else if (isPassword5G) {
                        // Update password 5GHz saja
                        parameterPaths.password_5g.forEach(passPath => {
                            parameterValues.push([passPath, value, "xsd:string"]);
                        });
                        console.log(`Added password 5GHz parameters for ${onuType} ONU`);
                        console.log(`  - 5GHz password paths: ${parameterPaths.password_5g.length} parameters`);
                    } else {
                        // Fallback: jika path tidak jelas, gunakan logika lama
                        console.warn(`⚠️  Ambiguous password path detected: ${path}, using fallback logic`);
    ...
    async reboot(deviceId) {
        try {
            ...
            try {
                ...
                console.log(`✅ Reboot task created successfully`);
            } catch (taskError) {
                console.warn(`⚠️  Failed with connection_request, trying fallback:`, taskError.message);
                ...
                console.log(`✅ Fallback reboot task created`);
            }
            ...
            return {
                ...
                taskId: taskResponse.data._id,
                processingTime: totalProcessingTime,
                message: `Reboot task completed in ${totalProcessingTime}ms`
            };
        } catch (error) {
            ...
            return {
                ...
                message: `Failed to reboot device: ${error.message}`
            };
        }
    },
        ...
        for (const device of devices) {
            try {
                ...
                if (timeDiff > thresholdMs) {
                    const pppoeUsername = device?.VirtualParameters?.pppUsername?._value ||
    device?.InternetGatewayDevice?.WANDevice?.[1]?.WANConnectionDevice?.[1]?.WANPPPConnection?.[1]?.Username?._value ||
