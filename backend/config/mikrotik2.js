...
async function getInterfaceTraffic(interfaceName = 'ether1') {
    try {
        ...
        try {
            ...
            logger.error(`[mikrotik2] monitor-traffic command failed for interface=${interfaceName}`, { error: innerErr && (innerErr.stack || innerErr.message || String(innerErr)) });

            // Fallback: lookup interface list and retry
            try {
                ...
                if (found) {
                    ...
                    try {
                        ...
                        logger.error(`[mikrotik2] monitor-traffic fallback also failed for interface=${interfaceName} using ${fallbackName}`, { error: innerErr2 && (innerErr2.stack || innerErr2.message || String(innerErr2)) });
                        return { rx: 0, tx: 0 };
                    }
                } else {
                    return { rx: 0, tx: 0 };
                ...
                logger.error(`[mikrotik2] monitor-traffic fallback lookup failed for interface=${interfaceName}`, { error: lookupErr && (lookupErr.stack || lookupErr.message || String(lookupErr)) });
                return { rx: 0, tx: 0 };
            }
        }

        if (!res || !res[0]) {
            logger.warn(`[mikrotik2] monitor-traffic returned empty for interface=${interfaceName}`, { res });
            return { rx: 0, tx: 0 };
        }
        ...
        if (rx === 0 && tx === 0) {
            logger.warn(`[mikrotik2] monitor-traffic response missing expected numeric fields for interface=${interfaceName}`, { sample });
        }
        ...
        try {
            ...
            logger.error('[mikrotik2] Error getting interface traffic:', details);
        } catch (e) {
            logger.error('[mikrotik2] Error getting interface traffic (serialize failed):', String(error));
        }
        return { rx: 0, tx: 0 };
    }
}
