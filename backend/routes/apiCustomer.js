...
router.post('/trouble/report', async (req, res) => {
    try {
        ...
        if (!name || !phone || !location || !category || !description) {
            return res.status(400).json({
                success: false,
                message: 'All fields are required (name, phone, location, category, description)'
            });
        }
        
        // Generate unique report ID
        const reportId = `TKT-${Date.now()}-${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
        
        console.log(`🎫 New trouble report submitted:`, {
            reportId,
            phone,
            name,
            location,
        ...
        const reportData = {
            ...
            priority: category === 'Tidak Bisa Browsing' ? 'high' : 'medium'
        };
        
        // TODO: Save to database
        // await saveTroubleReport(reportData);
...
router.get('/trouble/reports', async (req, res) => {
    try {
        ...
        // const reports = await getTroubleReportsByPhone(phone);
        
        // Mock data for now
        const mockReports = [
            {
                reportId: 'TKT-1637123456-ABC12',
                category: 'Internet Lambat',
                status: 'in_progress',
                createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
                description: 'Internet sangat lambat sejak kemarin'
            },
            {
                reportId: 'TKT-1637023456-XYZ89',
                category: 'WiFi Tidak Muncul',
                status: 'resolved',
                createdAt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
                description: 'WiFi tidak terdeteksi di HP'
            }
        ];
