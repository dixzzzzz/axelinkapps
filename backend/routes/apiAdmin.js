...
async function getCachedGenieACSDevices() {
    ...
    if (genieacsDevicesCache.data && (now - genieacsDevicesCache.timestamp) < genieacsDevicesCache.ttl) {
        console.log('📋 Using cached GenieACS devices data');
    ...
    console.log(`📋 Cached ${devices.length} GenieACS devices for 60 seconds`);
...
router.get('/genieacs/devices', async (req, res) => {
    ...
    try {
        console.log('🔄 Getting GenieACS devices for admin management...');
        ...
        const devices = devicesRaw.map((device) => {
            ...
            console.log(`🔍 Device ${device._id}: lastInform=${device._lastInform}, minutesDiff=${minutesDiff.toFixed(1)}m, hoursDiff=${hoursDiff.toFixed(2)}h`);
                ...
                password5g: (() => {
                    ...
                    if (ssid5g && ssid5g !== '') {
                        ...
                        if (password2g && password2g !== '-') {
                            console.log(`🔍 Using 2.4GHz password as 5GHz fallback for device ${device._id}`);
                            return password2g;
                        }
                    }
        ...
        console.log(`✅ Processed ${devices.length} devices for admin management`);
...
router.post('/settings/save', (req, res) => {
    ...
    try {
        ...
        if (!newSettings.admin_username || !newSettings.admin_password) {
            return res.status(400).json({
                success: false,
        ...
        // Since settings are now in .env, we can't dynamically save them
        // But we can update the cache for the current session
        ...
        res.json({
            success: true,
            message: 'Settings updated for current session. Note: Configuration is managed via .env file - restart required for permanent changes.',
            data: newSettings,
            warning: 'Settings are now managed via environment variables (.env file). Changes will only persist for the current session.'
        });
