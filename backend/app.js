...
// Mount Mikrotik management routes for both API and server-rendered admin pages
app.use('/api/admin', adminMikrotikRouter);
...
global.appSettings = {
    ...
    genieacsUrl: getSetting('genieacs_url', 'your-genieacs-url'),
    genieacsUsername: getSetting('genieacs_username', 'your-username'),
    genieacsPassword: getSetting('genieacs_password', 'your-password'),
    mikrotikHost: getSetting('mikrotik_host', 'your-mikrotik-host'),
    mikrotikPort: getSetting('mikrotik_port', 'your-mikrotik-port'),
    mikrotikUser: getSetting('mikrotik_user', 'your-mikrotik-user'),
    mikrotikPassword: getSetting('mikrotik_password', 'your-mikrotik-password'),
...
(async function initializeServices() {
    try {
        ...
        // Attempt to wire sock regardless of return value from connectToWhatsApp
        const wireSock = () => {
            const activeSock = whatsapp.getSock?.() || global.whatsappSock;
            if (!activeSock) {
                logger.warn('⚠️ WhatsApp sock not available, will retry wiring');
                return false;
            }
            try {
                // Ensure WhatsApp module is aware (redundant safe call)
                whatsapp.setSock(activeSock);
            } catch (e) {}
            try {
                const { setSockInstance: setTroubleSock } = require('./config/troubleReport');
                if (typeof setTroubleSock === 'function') {
                    setTroubleSock(activeSock);
                    logger.info('🔗 WhatsApp sock wired to troubleReport module');
                }
            } catch (e) {
                logger.warn('⚠️ Failed to wire WhatsApp sock to troubleReport module:', e.message);
            }
            return true;
        };
