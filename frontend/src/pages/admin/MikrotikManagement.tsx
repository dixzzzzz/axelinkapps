...
export default function MikrotikManagement() {
  ...
  const [userForm, setUserForm] = useState<UserFormData>({ username: '', password: '', profile: '' });
  ...
  const fetchData = async () => {
    try {
      ...
      const profilesResponse = await fetch('/admin/mikrotik/profiles/api', {
        credentials: 'include'
      });
      ...
      const usersResponse = await fetch('/admin/mikrotik/users/api', {
        credentials: 'include'
      });
      ...
      const users: PPPoEUser[] = usersData.users.map((user: any) => ({
        ...
        ipAddress: user.ipAddress || user.address || (user.active ? '10.10.1.100' : '-'),
        uptime: user.uptime || (user.active ? '1h 30m' : '-'),
  ...
  const handleAddUser = () => {
    setEditingUser(null);
    setUserForm({ username: '', password: '', profile: '' });
  ...
  const handleSaveUser = async () => {
    if (!userForm.username || !userForm.password || !userForm.profile) {
    ...
    try {
      const endpoint = editingUser 
        ? '/admin/mikrotik/edit-user'
        : '/admin/mikrotik/add-user';
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      const response = await fetch('/admin/mikrotik/delete-user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleDisconnectUser = async (userId: string) => {
    ...
    try {
      ...
      const response = await fetch('/admin/mikrotik/disconnect-session', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">PPPoE User Management</h1>
              ...
              <TableBody>
                {filteredUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        <div className="text-sm">↓ {formatBytes(user.bytesIn)}</div>
                        <div className="text-sm">↑ {formatBytes(user.bytesOut)}</div>
                      </div>
                    </TableCell>
