...
export default function InfrastructureManagement() {
  ...
  const loadInfrastructureData = async () => {
    try {
      ...
      toast.error('Gagal memuat data infrastruktur');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const loadDevices = async () => {
      try {
        ...
        console.error('Gagal memuat devices:', e);
      }
    };
    loadDevices();
  }, []);
  ...
  const saveInfrastructure = async (isEdit: boolean = false) => {
    try {
      ...
      if (!infraForm.name || !parsedCoords) {
        toast.error('Harap isi nama dan koordinat yang valid');
        return;
      }
      ...
      if (data.success) {
        ...
        toast.success(`${infraType} berhasil ${isEdit ? 'diupdate' : 'ditambahkan'}`);
        ...
        throw new Error(data.message || `Gagal ${isEdit ? 'mengupdate' : 'menambahkan'} ${infraType}`);
      }
    } catch (error) {
      console.error('Error saving infrastructure:', error);
      toast.error(`Gagal ${isEdit ? 'mengupdate' : 'menambahkan'} ${infraType}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const deleteInfrastructure = async (infra: ODPLocation | ODCLocation) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus ${infra.type} "${infra.name}"?`)) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(`${infra.type} berhasil dihapus`);
        await loadInfrastructureData();
      } else {
        throw new Error(data.message || `Gagal menghapus ${infra.type}`);
      }
    } catch (error) {
      console.error('Error deleting infrastructure:', error);
      toast.error(`Gagal menghapus ${infra.type}`);
    }
  };
  ...
  const getStatusBadge = (status: string, type: string) => {
    if (status === 'active') {
      ...
      return <Badge variant="outline" className="border-blue-500 text-blue-600">Ekspansi</Badge>;
    }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-purple-200 mt-1">
                  {infraStats.odp.active} Active • {infraStats.odp.full} Penuh
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-indigo-200 mt-1">
                  {infraStats.odc.active} Active • {infraStats.odc.expansion} Ekspansi
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="ID, nama, atau alamat..."
                  value={searchTerm}
